...
一、Self-service
1. **Business Applications** - 业务应用程序 / ビジネスアプリケーション
   - 可以访问和管理各类业务应用程序。
2. **DashBoards** - 仪表板 / ダッシュボード
   - 数据和信息的可视化展示。
3. **Service Catalog** - 服务目录 / サービスカタログ
   - 提供各种服务的目录和服务请求功能。
4. **Employee Center** - 员工中心 / 従業員センター
   - 员工个人信息和服务管理中心。
5. **Knowledge** - 知识 / ナレッジ
   - 包括员工可以访问的知识库和信息资源。

6. **Visual Task Boards** - 可视化任务板 / ビジュアルタスクボード
   - 可视化任务管理和进度追踪。
7. **Incidents** - 事件 / インシデント
   - 管理和追踪各类事件和问题。
8. **Watched Incidents** - 关注的事件 / ウォッチしたインシデント
   - 用户关注和监视的事件列表。
9. **My Requests** - 我的请求 / マイリクエスト
   - 用户发起的请求和申请管理。
10. **Requested Items** - 请求的条目 / リクエストされたアイテム
    - 用户提交的各类请求项目。
11. **Watched Requested Items** - 关注的请求条目 / ウォッチしたリクエストされたアイテム
    - 用户关注和监视的请求项目。

12. **My Connected Apps** - 我连接的应用程序 / マイ接続アプリ
    - 用户连接和管理的应用程序。
13. **My Profile** - 我的个人资料 / マイプロファイル
    - 用户个人信息和设置管理。
14. **My Tagged Documents** - 我标记的文档 / タグ付きドキュメント
    - 用户标记和管理的文档。
15. **My Tags** - 我的标签 / マイタグ
    - 用户创建和管理的标签。
16. **My Knowledge Articles** - 我的知识文章 / マイナレッジ記事
    - 用户创建和管理的知识文章。
17. **Take Survey** - 参与调查 / アンケートに回答する
    - 参与各类调查和反馈。
18. **My Approvals** - 我的批准 / マイ承認
    - 用户的批准和审批管理。
19. **My Approvals And Surveys** - 我的批准和调查 / マイ承認とアンケート
    - 用户的批准和调查管理。
20. **My Assets** - 我的资产 / マイアセット
    - 用户资产和资源管理。
21. **My Notification Preferences** - 我的通知偏好 / マイ通知設定
    - 用户的通知偏好设置。

二、Access Analyzer
1. **Analyze Permissions** - 分析权限 / パーミッションの分析
   - 分析和审查权限设置。
2. **Access Analyzer Queries** - 访问分析器查询 / アクセスアナライザークエリ
   - 执行和管理访问分析器的查询。

三、Activity Subscriptions
...
7. **Actions** - 操作 / アクション
   ...
   - 7.1 **Playbook Actions** - 操作手册操作 / プレイブックアクション
     - 操作手册中的操作管理。
   - 7.2 **Stage Actions** - 阶段操作 / ステージアクション
     - 各阶段的操作管理。
...
10. **Administration** - 管理 / 管理
    ...
    - 10.4 **Change Properties** - 变更属性 / 変更プロパティ
      - 管理变更的属性设置。
    - 10.5 **Risk Properties** - 风险属性 / リスクプロパティ
      - 管理风险的属性设置。
...
40. **CI Lifecycle Management** - CI 生命周期管理 / CI ライフサイクル管理
    ...
    - 40.7 **Not Allowed Operational Transitions** - 不允许的操作转换 / 許可されない運用状態の遷移
      - 查看和管理不允许的操作转换。
    - 40.8 **State Management Logs** - 状态管理日志 / 状態管理ログ
      - 查看和管理状态管理日志。
...
7. **Catalog Administration** - 目录管理 / カタログ管理
   ...
   - 7.5 **Properties** - 属性 / プロパティ
     - 查看和编辑属性设置。
   - 7.6 **Request Parent Mapping** - 请求父级映射 / リクエスト親マッピング
     - 管理请求父级映射。
